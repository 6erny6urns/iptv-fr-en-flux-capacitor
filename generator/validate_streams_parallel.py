...
def main():
    ...
    # Filtrer par top channels/cities avant validation
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as logf, ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            if valid:
                ...
                logf.write(f"VALID [{i}/{total}]: {name} -> {url}\n")
                print(f"VALID [{i}/{total}]: {name}")
            else:
                logf.write(f"INVALID [{i}/{total}]: {name} -> {url}\n")
                print(f"INVALID [{i}/{total}]: {name}")
